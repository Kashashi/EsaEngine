#include "shader.h"
namespace Kashashi{
著色器::著色器(const GLchar* 頂點著色器路徑,
			const GLchar* 片段著色器路徑,
			std::shared_ptr<EsaLogger> loggerPtr):m_logger(loggerPtr){
    // 1. 從文件開啟頂點/片段著色器
    std::string 頂點著色器緩衝區;
    std::string 片段著色器緩衝區;
    ...
    try{
        ...
        (*m_logger).writeLog("ERROR::SHADER::FILE_NOT_SUCCESFULLY_READ");
    }
	...
	if(!success)
	{
		...
		(*m_logger).writeLog("ERROR::SHADER::頂點著色器::COMPILATION_FAILED\n" + std::string(infoLog));
	};
    ...
    if (!success){
        ...
        (*m_logger).writeLog("ERROR::SHADER::片段著色器::COMPILATION_FAILED\n" + std::string(infoLog));
    }

	// 著色器ID
	程序唯一識別碼 = glCreateProgram();
	glAttachShader(程序唯一識別碼, 頂點著色器);
	glAttachShader(程序唯一識別碼, 片段著色器);
	glLinkProgram(程序唯一識別碼);
	// 輸出錯誤（如果有的話）
	glGetProgramiv(程序唯一識別碼, GL_LINK_STATUS, &success);
	if(!success)
	{
		glGetProgramInfoLog(程序唯一識別碼, 512, NULL, infoLog);
		(*m_logger).writeLog("ERROR::SHADER::PROGRAM::LINKING_FAILED\n" + std::string(infoLog));
	}
